...
int App_Pnumber=0,App_number=0;
...
void parse_client_dataS(char content[],int App_number)
{
...
void parse_server_dataS(char content[],int App_number)
{
    /*
    char temp[65535];
    char str1[1024];
...
/*下面的函数是对浏览器接收的数据进行分析*/
void parse_client_data(char content[],unsigned int App_number)
{
    //printf("内容分析 GO !");
    char temp[65535];
    //HTTP协议
...
/*下面的函数是对WEB服务接收到的数据进行分析*/
void parse_server_data(char content[],unsigned int App_number)
{
    char temp[65535];
    char str1[1024];
    char str2[1024];
    ...
    for(i=0;i<strlen(content);i=i+1)
    {
        ...
        if((content[i]=='\n')&&(content[i+1]=='\r')&&(content[i+2]=='\n'))
        {
            ...
            for(j=0;j<App_number-i-3;j=j+1)
            {
                entity_content[j]=content[i+3+j];
            }
            entity_content[j]='\0';
